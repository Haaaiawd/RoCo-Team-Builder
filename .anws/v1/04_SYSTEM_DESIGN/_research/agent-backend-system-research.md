...
**核心问题**: 如何为 `agent-backend-system` 设计一个可被 Open WebUI 作为 OpenAI 兼容端点接入、可处理多轮上下文、兼容第三方 OpenAI 兼容模型、支持多模态图片透传、并满足 v1 单进程内存会话约束的后端系统。

**探索范围**:
- 包含: Open WebUI 集成契约、OpenAI Chat Completions 流式协议、Agents SDK provider/session 约束、Rich UI 沙箱边界、v1 部署与异常处理策略
- 不包含: `data-layer-system` 的具体 BWIKI schema、`spirit-card-system` 的 HTML 细节、前端 UI 配置细节
...
2. **v1 应强制使用 Chat Completions 路径，而不是 Responses API**: Agents SDK 官方模型文档与本地源码均表明，第三方 OpenAI 兼容 Provider 常常只支持 Chat Completions；`OpenAIProvider(base_url=..., use_responses=False)` 是必须约束，否则会误打 `/responses`。
...
4. **会话隔离应建立在 Open WebUI 转发头上，但不能只用 user id**: `X-OpenWebUI-User-Id` 能实现用户级隔离，但若同一用户并行开启多个聊天，仅用 user id 会串上下文。v1 最佳 key 为 `user_id + chat_id` 组合，缺失时再回退到 `user_id`。
...
**结论**:
- `agent-backend-system` 的 OpenAI 兼容面必须至少实现:
  - `GET /v1/models`
  - `POST /v1/chat/completions`
- 会话 key 设计必须消费这些 header，而不是自己发 token。

**来源**:
- 本地源码: `references/open-webui/backend/open_webui/routers/openai.py`
- 本地源码: `references/open-webui/backend/open_webui/env.py`
- 本地源码: `references/open-webui/backend/open_webui/utils/headers.py`
...
**结论**:
- v1 必须将 provider 初始化写死为 `use_responses=False`。
- 不应在 v1 中使用 `OpenAIResponsesCompactionSession` 等依赖 Responses API 的能力。

**来源**:
- Agents SDK Models 文档: https://openai.github.io/openai-agents-python/models/ （访问于 2026-04-07）
- 本地源码: `references/openai-agents-python/src/agents/models/openai_provider.py`
...
**来源**:
- 本地源码: `references/openai-agents-python/src/agents/models/openai_chatcompletions.py`
...
- `SQLiteSession` 即使默认 `:memory:` 也引入 sqlite 连接与表结构管理，本质仍是一个数据库实现，不适合作为 v1 默认最简路径。
- ADR-003 已明确：v1 使用非持久化内存 Session，`uvicorn --workers 1`。

**结论**:
- v1 采用自定义 `InMemorySessionStore` + `Session` 适配层，比直接引入 `SQLiteSession` 更简单、边界更清晰。
- 必须实现 30 分钟闲置清理与进程重启即丢失的显式语义。

**来源**:
- `.anws/v1/03_ADR/ADR_003_SESSION_MANAGEMENT.md`
- 本地源码: `references/openai-agents-python/src/agents/memory/session.py`
- 本地源码: `references/openai-agents-python/src/agents/memory/sqlite_session.py`
...
- OpenAI streaming 文档搜索摘要: https://developers.openai.com/api/docs/guides/streaming-responses （访问于 2026-04-07，正文抓取 403）
- 本地源码: `references/openai-agents-python/tests/test_openai_chatcompletions_stream.py`
...
- 本地源码线索: `references/open-webui/src/lib/stores/index.ts`
...
| P2 | 为 v2 预留 Redis Session 接口 | 后续多 worker 横向扩展时平滑升级 |
